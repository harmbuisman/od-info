...
from opsdata.ops import Ops, grab_search
...
def update_dom_index(session, db):
    ...
    for code, line in grab_search(session).items():
        ...
        if code not in doms:
            dom = Dominion(code=int(line["code"]), name=line["name"], realm=int(line["realm"]), race=line["race"])
            db.session.add(dom)
        else:
            dom = doms[line["code"]]

        dh = DominionHistory(
            dominion_id=int(line["code"]), timestamp=timestamp, land=int(line["land"]), networth=int(line["networth"])
        )
...
def update_ops(ops: Ops, db, dom_code):
    logger.debug("updater - Updating ops for dominion %s", dom_code)
    timestamp = ops.timestamp
...
def update_town_crier(session, db):
    ...
    for page_nr in range(1, get_number_of_tc_pages(session) + 1):
        ...
        for event in events:
            tc_event = TownCrier(
                timestamp=cleanup_timestamp(event[0]),
                origin=event[2],
                origin_name=event[3],
                target=event[4],
                target_name=event[5],
...
DOM_HISTORY_MAPPING = {"dominion": None, "timestamp": None, "land": "status.land", "networth": "status.networth"}
...
def query_stealables(db, timestamp, my_realm: int):
    params = {"timestamp": cleanup_timestamp(timestamp), "realm": my_realm}
...
def update_revelation(db, ops, dom):
    for spell in ops.q("revelation.spells"):
        if not db.session.get(Revelation, [dom.code, ops.timestamp, spell["spell"]]):
            obj = Revelation(
                dominion_id=dom.code, timestamp=ops.timestamp, spell=spell["spell"], duration=int(spell["duration"])
            )
