...
from config import SEARCH_PAGE, current_player_id
from domain.dataaccesslayer import all_doms, dom_by_id, is_database_empty, query_town_crier, realm_of_dom, realmies
from domain.models import Dominion
from domain.timeutils import add_duration, current_od_time, hours_since
from facade.awardstats import AwardStats
from facade.discord import send_to_webhook
from opsdata.ops import get_last_scans, grab_my_ops, grab_ops
from opsdata.scrapetools import get_soup_page, login, read_tick_time
from opsdata.updater import query_stealables, update_dom_index, update_ops, update_town_crier
...
class ODInfoFacade(object):
    ...
    def update_all(self):
        ...
        for dom in all_doms(self._db):
            ...
            if (domcode in last_scans) and ((dom.last_op is None) or (dom.last_op < last_scans[domcode])):
                print(f"Checking {domcode}: {dom.last_op} vs {last_scans.get(domcode, None)}")
    ...
    def update_ops(self, dom_code):
        logger.debug("update_ops - Updating ops for dominion %s", dom_code)
    ...
    def update_role(self, dom_code, role):
        ...
        qry = text("UPDATE Dominions SET role = :role WHERE code = :code")
    ...
    def update_player(self, dom_code, player_name):
        ...
        qry = text("UPDATE Dominions SET player = :name WHERE code = :code")
    ...
    def military_list(self, versus_op=0, top=20):
        ...
        for mc in mc_list:
            ...
            dom_result = {
                ...
                "paid_until": "?" if mc.army is None else mc.army.get("paid_until", "?"),
                "raw_op": mc.raw_op,
                "op": mc.op,
                "raw_dp": mc.raw_dp,
                "dp": mc.dp,
                ...
                "offense_bonus": mc.offense_bonus,
                "defense_bonus": mc.defense_bonus,
                "networth": mc.dom.current_networth,
            }
            result_list.append(dom_result)

        if max_land > 0:
            result_list_2 = [dom for dom in result_list if dom["land"] < max_land]
